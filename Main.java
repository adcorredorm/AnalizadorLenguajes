public class Main{
	...
	public static boolean pudo_abrirse_fuente(){
		String nomb_fuente;
		double p_archivo;
		boolean ok;
	...
	public static boolean es_letra(String c){
		boolean r;
		
		r = (c>='a'andc<='z')or(c>='A'andc<='Z');

		return r;
	}

	public static boolean es_digito(String c){
		boolean r;
	...
	public static boolean es_palabra_reserv(String s, boolean ipr){
		double k;
		boolean enc;
		
		enc = NO;

		for(int k = 1; k < alen(pal_reserv); k++){
			if(s==pal_reserv[k].pal){
				ipr = pal_reserv[k];
	...
	public static String leer_constante_entera(){
		...
		return substr(lt,pos_i,(up-pos_i)+1);
	}
